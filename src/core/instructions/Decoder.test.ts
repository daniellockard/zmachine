...
describe('Decoder', () => {
  ...
  describe('Variable form instructions', () => {
    ...
    it('should decode storew with 3 operands', () => {
      ...
      const memory = createMemoryWithBytes([
        ...
        0x10,
        0x00, // Operand 1: large constant 0x1000
  ...
  describe('Print instructions with inline text', () => {
    it('should throw if text decoder not set', () => {
      // print: 0xB2 followed by Z-string
      const memory = createMemoryWithBytes([
        0xb2, // print
        0x80,
        0x00, // Minimal Z-string (just end marker)
      ]);
      const decoder = new Decoder(memory, 3);
      // Don't set text decoder

      expect(() => decoder.decode(0x100)).toThrow('Text decoder not set');
    });

    it('should decode print instruction with text decoder', () => {
      const memory = createMemoryWithBytes([
        0xb2, // print
        0x80,
        0x00, // Minimal Z-string
      ]);
      const decoder = new Decoder(memory, 3);
      decoder.setTextDecoder(() => ({ text: 'Hello', bytesConsumed: 2 }));
