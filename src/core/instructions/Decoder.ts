...
export class Decoder {
  ...
  private determineForm(opcodeByte: number): InstructionForm {
    ...
    if (opcodeByte === 0xBE && this.version >= 5) {
      return InstructionForm.Extended;
    }
    
    // Variable: top 2 bits = 11
    if ((opcodeByte & 0xC0) === 0xC0) {
  ...
  private decodeLong(address: ByteAddress, offset: number, opcodeByte: number): DecodedInstruction {
    const opcode = opcodeByte & 0x1F;
    const opcodeInfo = get2OPInfo(opcode);
    
    // Decode operand types from bits 6 and 5
    const type1 = (opcodeByte & 0x40) ? OperandType.Variable : OperandType.SmallConstant;
    const type2 = (opcodeByte & 0x20) ? OperandType.Variable : OperandType.SmallConstant;
   ...
   */
  private decodeShort(address: ByteAddress, offset: number, opcodeByte: number): DecodedInstruction {
   ...
   */
  private decodeVariable(address: ByteAddress, offset: number, opcodeByte: number): DecodedInstruction {
    const opcode = opcodeByte & 0x1F;
    const isVAR = (opcodeByte & 0x20) !== 0;
    
    // Read operand type byte(s)
    // VAR opcodes 0x0C and 0x1A have two type bytes (up to 8 operands)
    const hasDoubleTypes = isVAR && (opcode === 0x0C || opcode === 0x1A);
  ...
  private finishDecode(
    ...
    if (opcodeInfo?.branches) {
      ...
      const branchOnTrue = (branchByte1 & 0x80) !== 0;
      ...
      if (branchByte1 & 0x40) {
        ...
        // 14-bit signed value
        branchOffset = ((branchByte1 & 0x3F) << 8) | branchByte2;
        // Sign-extend from 14 bits
        if (branchOffset & 0x2000) {
          branchOffset = branchOffset - 0x4000;
        }
      }
    ...
    if (opcodeInfo?.hasText) {
      if (this.textDecoder) {
        ...
        while (true) {
          ...
          offset += 2;
          if (word & 0x8000) break; // High bit marks end of string
        }
   ...
   */
  private readOperand(address: ByteAddress, type: OperandType): { value: number; bytesRead: number } {
    switch (type) {
