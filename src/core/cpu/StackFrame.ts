...
export class StackFrame {
  ...
  constructor(
    ...
    if (localCount < 0 || localCount > 15) {
      throw new StackError(`Invalid local variable count: ${localCount} (must be 0-15)`);
    }
  ...
  pop(): Word {
    if (this.evalStack.length === 0) {
      throw new StackError('Stack underflow: cannot pop from empty evaluation stack');
    }
  ...
  peek(): Word {
    if (this.evalStack.length === 0) {
      throw new StackError('Stack underflow: cannot peek at empty evaluation stack');
    }
